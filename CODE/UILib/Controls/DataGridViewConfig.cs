...
namespace ORG.UILib.Controls
{
    public abstract class DataGridViewConfig
    {
        ...
        /// <returns></returns>
        public static DataSet ReadDGVConfig(string username, string nodeName)
        {
            lock (writeFile)
            {
                ...
                try
                {
                    string fileName = "DataGridViewColumnsConfig";
        ...
        private static DataSet ReadXmlUserSet(string username, string nodeName)
        {
            ...
            {
                dt.Columns.Add(xmlattrbute.Name);
            }
            //行数据
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static bool WriteDGVConfig(string username, string nodeName, DataTable dt)
        {
            lock (writeFile)
            {
                bool iftrue = false; ;
        ...
        private static bool WriteXmlUserSet(string username, string nodeName, DataTable dt, string xmlFilePath)
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        xeRow.SetAttribute(col.ColumnName, dr[col.ColumnName].ToString());
                    }
