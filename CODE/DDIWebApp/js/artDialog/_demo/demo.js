...
(function(){
	...
	domReady(function(){
		...
		$('btn1').onclick = function(){
			window.top.art.dialog({title:'图片查看',fixed:true, content:'<img width="817" height="479" src="http://www.hunanyishi.cn/images/main.jpg" />'});
			return false;
		};
		

		$('btn2').onclick = function(){
			window.top.art.dialog({id:'testIframe', iframe:'_demo/iframe.html' }, function(){
				var msg = window.top.art.dialog({id:'testIframe'}).data.iframe;// 使用内置接口获取iframe对象
		...
		$('btn5').onclick = function(){
			window.top.art.dialog({content:'你是坏人', style:'succeed', width:'14em', lock:true}, function(){
				window.top.art.dialog({content:'hello world!', lock:true});
		...
		$('btn6').onclick = function(){
			...
			if ($('menu_4834783')) {
				window.top.art.dialog({id:'menu_4834783'}).close();
				_this.innerHTML = '弹出菜单';
				return;
			};
			
			window.top.art.dialog({id:'menu_4834783', title:'菜单', content:'请输入：<input style="width:200px;" id="M_dfd" type="text" value="hello world!" />',menuBtn:this}, function(){
				var a = $('M_dfd').value;
				window.top.art.dialog({content:a, lock:true, time:1});
			}).close(function(){
				_this.innerHTML = '弹出菜单';
			});
		...
		$('btn6runCodeBtn').onclick = function(){
			window.top.art.dialog({id:'dg_btn6runCodeBtn', content:'你确定运行吗？', menuBtn:$('runCodeBtn')}, function(){
				$('runCodeBtn').click();
			}, function(){});
		};
		
		$('btn7').onclick = function(){
			window.top.art.dialog({mouse:true, id:'dg_test34243', content:'您收到 <strong>2</strong> 条消息',left:'right',width:'15em', top:'bottom', fixed:true});//left和top坐标可以使用关键字，当然也可以使用数字
			return false;
		};
		$('btn7Close').onclick = function(){
			window.top.art.dialog({id:'dg_test34243'}).close();
			return false;
		};
		//--------------------------ardDialog演示脚本结束------------------------------//
		...
		var demoNav = function(){
			...
			window.top.art.dialog({id:'pageNav', content:html, fixed:true, top:'bottom', style:'noSkin'})
		}();
		...
		$('runCodeBtn').onclick = function(){
			...
			try {
				...
				window.top.art.dialog({title:'警告', content:'悟空，不要乱搞！<br />' + e, lock:true}).close(function(){
					$('runCode').value = "window.top.art.dialog({content:'hello world!'})";
				});
			};
			return false;
		};
...
//显示一个新对话框
function showWin(){
			window.top.art.dialog({content:'欢迎使用 "window.top.art.dialog" 对话框组件！', fixed:true, lock:false,style:'succeed'}, function(){window.top.art.dialog({title:' ', content:'谢谢观赏', lock:true, time:2})});
};
